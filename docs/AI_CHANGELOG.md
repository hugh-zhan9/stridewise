...
- (未检测到变更文件或当前目录非 git 仓库)
----------------------------------------
## [2026-03-11 16:49] [Feature]
- **Change**: 推荐输入加入最近训练反馈
- **Risk Analysis**: 影响推荐输入结构，可能引起模型输出变化；已在 backend 目录完成 go test ./... 回归。
- **Risk Level**: S2（中级: 局部功能异常、可绕过但影响效率）
- **Changed Files**:
- (未检测到变更文件或当前目录非 git 仓库)
----------------------------------------
...
- `.gitignore`
----------------------------------------
## [2026-03-11 19:16] [Feature]
- **Change**: 训练重算编排与worker依赖注入
- **Risk Analysis**: 训练重算流程变更可能影响异步任务稳定性；新增worker推荐生成依赖可能在配置缺失时触发回退或失败。
- **Risk Level**: S2（中级: 局部功能异常、可绕过但影响效率）
- **Changed Files**:
- (未检测到变更文件或当前目录非 git 仓库)
----------------------------------------
## [2026-03-11 19:25] [Feature]
- **Change**: 训练记录手动限制与训练重算入队
- **Risk Analysis**: 训练记录更新/删除逻辑调整可能影响现有调用方，错误时将返回 400；入队 training:recalc 改变异步任务流转路径。
- **Risk Level**: S2（中级: 局部功能异常、可绕过但影响效率）
- **Changed Files**:
- (未检测到变更文件或当前目录非 git 仓库)
----------------------------------------
